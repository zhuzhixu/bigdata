...
public class JobCleanService implements Service {
	...
	public synchronized ServiceState process() {/////
		try {
			...
			String tableString = hbaseclassify.getProperty("hbasetable");
			String[] tStrings = tableString.split(",");
			String config = ReadFile.ReadFile(System.getProperty("user.dir")+ "/configuration/hbaseclassify.json");
			JSONObject jsonObject = new JSONObject(config);
			for(int i = 0 ; i< tStrings.length; i++) {
				JSONArray array = jsonObject.getJSONArray(tStrings[i]);
				jobDataReposity.classify(array, "job_"+tStrings[i], tStrings[i]);
			}
